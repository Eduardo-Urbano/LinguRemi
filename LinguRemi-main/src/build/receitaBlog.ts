...
document.addEventListener("DOMContentLoaded", () => {
    const cont1 = document.getElementById("receitaBlog") as HTMLElement;
    if (!cont1) return;
    ...
    function preencherReceita(){
        ...
        receitasDoce.forEach((receita) => {
            const listaIng = receita.ingredientes.split(",");
            const listaHTML = listaIng.map(item => `<li>${item.trim()}</li>`).join("");
